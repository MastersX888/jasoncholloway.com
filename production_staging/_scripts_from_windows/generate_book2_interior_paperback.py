...
"""
Masters X Book 2 Interior PDF Generator v5
Anathem-Inspired Typography · IngramSpark Upload

v5 fixes (2026-05-11, mirrored from Book 3 v5 build):
- Added a dedicated part-title page (P7) with "VOLUME TWO" eyebrow, no
  running head or folio, plus a quiet verso (P8) behind it.
...
- Added an "END OF VOLUME TWO - THE GRIMOIRE" closer page after backmatter.
- Decimal-Hz frequency subtitles ("3.915 Hz - Iceland", "110.4 Hz - ...")
  now classify correctly so they render in chapter-subtitle style.
- Ornament markers tolerated with either single or double spaces between
  glyphs (the canonical DOCX uses single spaces in places, double in others).
- Set the assigned Bowker ISBN (979-8-2958-1267-5) as the default for the
...
- Coda-subtitle dedup carried over from Book 1/3.
...
OUTPUT_DIR = Path(r"C:\Users\zh577\.gemini\antigravity\scratch\jasoncholloway\Jason_Carroll_Holloway_Final_Export") #  / "Standard_BW_Paperback" / "Book2_The_Grimoire"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_PDF = Path(BUILD_OUTPUT) if BUILD_OUTPUT else (OUTPUT_DIR / "INTERIOR_MASTERS_X_BOOK2_PB.pdf")

# Standard B&W paperback Bowker ISBN for Volume Two.
BOOK_ISBN = (
    os.environ.get("MASTERSX_STANDARD_PB_BOOK2_ISBN", "").strip()
    or "979-8-2560-0995-3"
)
...
def classify(text):
    ...
    # Frequency subtitle now also accepts decimal Hz values ("3.915 Hz - Iceland").
    if re.match(r"^\d+(?:\.\d+)? Hz \u00b7 .+$", text): return "hz"
    if text == "MASTERS X": return "skip"
    if text == "MASTERS X: THE GRIMOIRE": return "skip"
...
def frontmatter():
    ...
    el.append(Paragraph("Volume Two", ParagraphStyle("V", parent=S_SUB, fontSize=11, textColor=LT)))
    ...
    el.append(Paragraph(
        ...
        "The preparation is about the organism.\u201d", S_EPIG))
    el.append(Spacer(1, 14))
    el.append(Paragraph("\u2014 Blake Masters, Moleskine IX",
    ...
    el.append(Paragraph("VOLUME TWO", S_PARTEYE))
    el.append(OrnRule(TEXT_W))
    el.append(Spacer(1, 18))
    el.append(Paragraph("MASTERS X", S_PARTTITLE))
    el.append(Spacer(1, 4))
    el.append(Paragraph("The Grimoire", S_PARTSUB))
    el.append(Spacer(1, 18))
    el.append(OrnRule(TEXT_W))
    el.append(PageBreak())
...
def preview_section(paras):
    ...
    el.append(Spacer(1, 4))
    el.append(Paragraph("Volume Three", ParagraphStyle("V2", parent=S_SUB, fontSize=11, textColor=LT)))
    ...
    el.append(Spacer(1, 8))
    el.append(Paragraph("The Kingdom continues at your bookseller.",
...
def closer_page():
    ...
    el.append(Spacer(1, TEXT_H * 0.42))
    el.append(Paragraph("END OF VOLUME TWO", S_PARTEYE))
    el.append(OrnRule(TEXT_W))
    el.append(Spacer(1, 14))
    el.append(Paragraph("MASTERS X", S_PARTTITLE))
    el.append(Spacer(1, 4))
...
def main():
    print("=" * 60)
    print("MASTERS X \u00b7 BOOK 2 \u00b7 INTERIOR PDF GENERATOR v5")
    print("Anathem Register \u00b7 IngramSpark Upload")
    print("=" * 60)

    print("\n[1/5] Extracting Book 2 with formatting...")
    book2 = extract_with_formatting(DOCX_PATH, 0, None)
    italic_count = sum(1 for p in book2 if "<i>" in p["rich"])
    print(f"      {len(book2)} paragraphs ({italic_count} with italic runs)")

    print("[2/5] Loading Book 3 preview from corrected V2 markdown...")
    b3_ch1_path = Path(r"E:\Masters_X_Trilogy_Archive\Old_Drafts_and_Backups\Book 3\CH01_THE_RETURN_V2.md")
    b3_text = b3_ch1_path.read_text(encoding="utf-8")
    preview = []
    for line in b3_text.split("\n"):
        s = line.strip()
    ...
    print(f"      {len(preview)} paragraphs (from {b3_ch1_path.name})")
    ...
    doc1.build(_build_elements(book2, []))  # preview disabled (live CANON has no preview)
    ...
    doc.build(_build_elements(book2, []))  # preview disabled (live CANON has no preview)
