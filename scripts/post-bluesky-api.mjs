#!/usr/bin/env node
/**
 * Post blog social copy to Bluesky via AT Protocol API.
 * Usage: node scripts/post-bluesky-api.mjs <slot> [--dry-run]
 */
...
function loadEnv() {
  ...
  throw new Error("BLUESKY_HANDLE and BLUESKY_APP_PASSWORD not found in .env");
}
...
async function main() {
  ...
  if (!slot) {
    console.error("Usage: node scripts/post-bluesky-api.mjs <slot> [--dry-run]");
    process.exit(1);
  }
  ...
  console.log(`Slot ${slot} Bluesky post (${text.length} chars)`);
  console.log("---");
  console.log(text);
  console.log("---");
