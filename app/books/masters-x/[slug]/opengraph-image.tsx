...
export function generateStaticParams() {
  return books.filter((b) => b.series === "Masters X").map((b) => ({
    slug: b.slug,
  }));
}
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '5rem', width: '100%', height: '100%' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', color: '#f5f0e8', flex: 1, justifyContent: 'center' }}>
            <div style={{ fontSize: '28px', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#8a6d3b', marginBottom: '1.5rem' }}>
              {book.series}
            </div>
            <div style={{ fontSize: '76px', fontWeight: 'bold', lineHeight: 1.1, marginBottom: '1.5rem' }}>
              {book.title}
            </div>
            {book.subtitle && (
              <div style={{ fontSize: '42px', color: '#a0a0a0', marginBottom: '3rem', fontStyle: 'italic' }}>
                {book.subtitle}
              </div>
            )}
