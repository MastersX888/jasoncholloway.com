...
| Omnibus case cover photo (the design under the dust jacket) | The site only ever showed the dust jacket — the case art stamped on the boards (the gold radial resonance diagram seen in the author's Facebook post) was never on the site at all. `components/store/CaseCoverReveal.tsx` now renders a jacket ⇄ case toggle on the homepage buy section and the omnibus product page, but shows a "photograph coming soon" placeholder until the real photo exists | Export/photograph the case (flat, even lighting, isolated on black like the other cover art — a straight product shot, not the styled Facebook flat-lay with the candle/quill/comp title), save as `public/media/omnibus-case-cover.jpg`, then set `caseCoverImage: "/media/omnibus-case-cover.jpg"` on the `omnibus` entry in `lib/data/books.ts` — no other code changes needed |
